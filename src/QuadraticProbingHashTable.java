...
public class QuadraticProbingHashTable<K, V> implements MapADT<K, V>, Iterable<Map.Entry<K,V>> {
    private int c1;
    private int c2;
    ...
    public boolean insert(K key, V value) {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            if (table[bucketIndex].isEmpty()) {
                ...
                size++;  // added to increment the asize
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
        return new QuadProbingHashTableIterator();
    }

    public class QuadProbingHashTableIterator implements Iterator<Map.Entry<K, V>> {
        private int index = -1;

        public QuadProbingHashTableIterator() {
            index = -1;

            //  finding  the first valid bucket
        ...
        public boolean hasNext() {
            // Checking if there's a valid next element
            int temp = index;

            while (temp < table.length) {
                if (!table[temp].isEmpty()) {
                    return true;
                }
                temp++;
            }
            ...
            public Map.Entry<K, V> next () {
                while (index < table.length) {
                    if (!table[index].isEmpty()) {
                        Map.Entry<K, V> entry = new AbstractMap.SimpleEntry<>(table[index].key, table[index].value);
                        index++;

                        // Moving index to  nxt valid bucket
                        while (index < table.length && table[index].isEmpty()) {
                            index++;
                        }
                        return entry;
                    }
